...
$(document).on('click', '.share-post', function(e) {
  e.preventDefault();
  $(this).next('.share-options').toggleClass('show');
});
$(document).on('click', function(e) {
  if ($('.share-options').has(e.target).length === 0 && $('.share-post').has(e.target).length === 0) {
    $('.share-options').removeClass('show');
  }
});*/

setTimeout(function() { 
  $('.trustpilot-review-slider').slick({
    dots: true,
    infinite: true,
    arrows: false,
    infinite: true,
...
const readingTimeSummary = document.querySelector(".read-time");
...
if ($('body.template-product').length) { 
  var link = $('[data-pf-type="Image"]');
  jQuery.each( link, function( index ) {
    var obj = $(this);  
    var linkString = $(obj).attr('src');                        
    if (typeof linkString !== 'undefined' && linkString.indexOf('Satisfaction') != -1) {
  ...
  //if($selectedFilters.length === 0 || ($selectedFilters.length === 1 && $selectedSort.length == 1)) {
    ...
    if($(window).width() > 768) {
      $('html, body').stop().animate({
          'scrollTop': $('.collection-matrix').offset().top - 210
      }, 500, 'swing');
    }
    ...
    $products.each((ii, product) => { 
      ...
      $selectedFilters.each((i, filter) => { 
        ...
        if(typeof filterCategory !== 'undefined' && filterCategory === 'ingredients') {
            IngredientsArr.push(filterValue);
            var productTags = productFilters.split('|');
            var intersection = productTags.filter(element => IngredientsArr.includes(element));
...
var t = function () {
  ...
  function n(t, i) {
    for (var n = t.charAt(0), o = i.split("&"), s = [], r = {}, a = [], l = t.substring(1), c = 0, h = o.length; c < h; c++)
      if ((s = o[c].match(/(.*?)=(.*)/)) || (s = [o[c], o[c], ""]), "" !== s[1].replace(/\s/g, "")) {
        if (s[2] = e(s[2] || ""), l === s[1]) return s[2];
  ...
  return function (e, o) {
    var s, r = {};
    if ("tld?" === e) return t();
    if (o = o || window.location.toString(), !e) return o;
    if (e = e.toString(), s = o.match(/^mailto:([^\/].+)/)) r.protocol = "mailto", r.email = s[1];
    else {
      if ((s = o.match(/(.*?)\/#\!(.*)/)) && (o = s[1] + s[2]), (s = o.match(/(.*?)#(.*)/)) && (r.hash = s[2], o = s[1]), r.hash && e.match(/^#/)) return n(e, r.hash);
      if ((s = o.match(/(.*?)\?(.*)/)) && (r.query = s[2], o = s[1]), r.query && e.match(/^\?/)) return n(e, r.query);
      if ((s = o.match(/(.*?)\:?\/\/(.*)/)) && (r.protocol = s[1].toLowerCase(), o = s[2]), (s = o.match(/(.*?)(\/.*)/)) && (r.path = s[2], o = s[1]), r.path = (r.path || "").replace(/^([^\/])/, "/$1"), e.match(/^[\-0-9]+$/) && (e = e.replace(/^([^\/])/, "/$1")), e.match(/^\//)) return i(e, r.path.substring(1));
      if ((s = i("/-1", r.path.substring(1))) && (s = s.match(/(.*?)\.(.*)/)) && (r.file = s[0], r.filename = s[1], r.fileext = s[2]), (s = o.match(/(.*)\:([0-9]+)$/)) && (r.port = s[2], o = s[1]), (s = o.match(/(.*?)@(.*)/)) && (r.auth = s[1], o = s[2]), r.auth && (s = r.auth.match(/(.*)\:(.*)/), r.user = s ? s[1] : r.auth, r.pass = s ? s[2] : void 0), r.hostname = o.toLowerCase(), "." === e.charAt(0)) return i(e, r.hostname);
      t() && ((s = r.hostname.match(t())) && (r.tld = s[3], r.domain = s[2] ? s[2] + "." + s[3] : void 0, r.sub = s[1] || void 0)), r.port = r.port || ("https" === r.protocol ? "443" : "80"), r.protocol = r.protocol || ("443" === r.port ? "https" : "http")
    }
...
"function" == typeof window.define && window.define.amd ? window.define("js-url", [], (function () {
  return t
})) : ("undefined" != typeof window.$ && window.$.extend({
  url: function (t, e) {
...
Shopify.theme.jsCollection = {
  init: function e() { 
      ...
      var i = url("?sort_by"); 
      if (null != i) {
          var o = $(`.sort-item[data-value="${i}"]`).find(".option-label").text();
          $(".sort-item").removeClass("selected"), $(`.sort-item[data-value="${i}"]`).addClass("selected"), $("#current-sort").text(o)
      }
      $("#collection-sort").click((() => {
          $("#sort-dropdown, #sort-triangle").slideToggle()
      })), $(".sort-item").click((function() {
          var e = $(this).find(".option-label").text();
          $(".sort-item").removeClass("selected"), $(this).addClass("selected"), $("#current-sort").text(e), $("#sort-dropdown, #sort-triangle").slideUp(), Shopify.theme.jsCollection.filterURL()
      })), $("div.filter.sort").click((function() {
          var e = $(this).data("value");
          $(".sort-item").removeClass("selected"), $(`.sort-item[data-value="${e}"]`).addClass("selected"), Shopify.theme.jsCollection.filterURL()
      }))
  },
  filterURL: function e() {
      var t = [],
          i = "",
          o = "",
          l = "https://" + $.url("hostname"),
          r = $.url("1") ? "/" + $.url("1") + "/" : "",
          a = $.url("2") ? $.url("2") + "/" : "",
          n = $.url("3"),
          s = r + a;
      i = $(".sort-item.selected").length ? $(".sort-item.selected").data("value") : url("?sort_by"), Shopify.theme.queryParameters.sort_by = i, i = "?" + $.param(Shopify.theme.queryParameters), this.processUrl(s, "", i)
  },
  processUrl: function e(t, i, o) {
      var o, l = "";
      l = t + i + (o = o.replace(/\page=(\w+)&/, "")), this.updateView(l)
  },
  updateView: function e(t) {
      $.ajax({
          ...
          beforeSend: function e() {
              $(".collection-matrix").addClass("animated loading-in-progress"), $("#sort-spinner").fadeIn()
          },
          success: function e(t) {},
          error: function e(i, o, l) {
              console.log(i), console.log(o), console.log(l), location.replace(location.protocol + "//" + location.host + t)
          },
          dataType: "html"
      }).then((function(e) {
          var i = $(".collection-matrix"),
              o = $(".collection__main");
          $("#sort-spinner").fadeOut(), i.removeClass("loading-in-progress");
          var l = $(e).find(".collection-matrix");
          var r = $(e).find(".container--no-products-in-collection");
          l.length ? (o.empty(), o.append(l), $("#filter-clicker").click()) : (o.empty(), o.append(r)), window.history && window.history.pushState && window.history.pushState("", "", t)
      }))
  },
  unload: function e(t) {
      $("#tag_filter, #sort-by").off(), $("[data-option-filter]").off(), $("[data-reset-filters]").off(), $("[data-clear-filter]").off()
  }
};
Shopify.theme.jsCollection.init();
